...
// 查询分类下的所有文章
func GetCateArt(cid, pageSize, pageNum int) ([]Article, int, int64) {
	var cateArts []Article
	var total int64
	if pageNum != -1 {
		err = db.Preload("Category").Limit(pageSize).Offset((pageNum-1)*pageSize).Where("cid = ?", cid).Find(&cateArts).Count(&total).Error
